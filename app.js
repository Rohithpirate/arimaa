...
const WHATSAPP_NUMBER = "919488727012";
const RAZORPAY_KEY_ID = "rzp_test_SpI0Sg4z3V1U9Y";
...
function updateUPIPayLink() {
  ...
  if (amountText) amountText.textContent = `Rs. ${total.toFixed(2)}`;
...
function setPaymentBehavior() {
  ...
  const apply = () => {
    ...
    const isUPI = selected && selected.value === "upi";
    const isRazorpay = selected && selected.value === "razorpay";
    upiBox.style.display = isUPI ? "block" : "none";
    if (razorpayBox) razorpayBox.style.display = isRazorpay ? "block" : "none";
    updateUPIPayLink();
  };
...
function setupRazorpayButton() {
  ...
  btn.addEventListener("click", async () => {
    const cart = readCart();
    if (!cart.length) {
      msg.textContent = "Cart is empty. Please add products before payment.";
      return;
    }

    if (!form.reportValidity()) return;
    if (!RAZORPAY_KEY_ID) {
      msg.textContent = "Razorpay Key ID missing.";
      return;
    }
    ...
    const email = "arimaaproducts@gmail.com";

    const options = {
      key: RAZORPAY_KEY_ID,
      amount: Math.round(total * 100),
      currency: "INR",
      name: "ARIMAA PRODUCTS",
      description: "Soap Order Payment",
      handler: function (response) {
        msg.textContent = `Payment success. Payment ID: ${response.razorpay_payment_id}`;
      },
  ...
  btn.addEventListener("click", () => {
    ...
    try {
      ...
      setTimeout(() => {
        if (msg) {
          msg.textContent =
            "If UPI app did not open, scan QR or copy UPI ID + amount and pay manually.";
        }
      }, 1200);
    } catch (_e) {
      if (msg) {
        msg.textContent =
          "Unable to open UPI app here. Please scan QR or copy UPI ID and pay manually.";
      }
    }
  });
}

function setupUPIFallbackButtons() {
  const upiBtn = document.getElementById("copyUpiIdBtn");
  const qrBtn = document.getElementById("downloadQrBtn");
  const msg = document.getElementById("order-msg");
  const flashButtonText = (btn, text = "Copied") => {
    if (!btn) return;
  ...
  upiBtn?.addEventListener("click", async () => {
    try {
      ...
      flashButtonText(upiBtn, "Copied");
      if (msg) msg.textContent = "UPI ID copied.";
    } catch (_e) {
      if (msg) msg.textContent = "Could not copy UPI ID. Please copy manually: Rohithpirate@iob";
    }
  });

  qrBtn?.addEventListener("click", () => {
    const link = document.createElement("a");
    link.href = "assets/images/upi-qr.jpeg";
    link.download = "arimaa-upi-qr.jpeg";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    if (msg) msg.textContent = "QR downloaded. You can scan and pay.";
  });
}
...
function setupOrderForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const formData = new FormData(form);
    const customer = String(formData.get("name") || "");
    const phone = String(formData.get("phone") || "");
    const address = String(formData.get("address") || "");
    const payment = String(formData.get("payment") || "upi");
    const total = getTotal(cart);
    const itemsText = cart.map((item) => `${item.name} x${item.qty} = Rs. ${item.price * item.qty}`).join("\n");

    const message = [
      "Hello ARIMAA PRODUCTS, I want to place my order.",
      `Name: ${customer}`,
      `Phone: ${phone}`,
      `Address: ${address}`,
    ...
    msg.textContent = `Thank you, ${customer}. WhatsApp message opened with your order details.`;
  });
}
